...
def start_dummy(host, port, server_class=HTTPServer, handler_class=S):
  try:
    ...
    if check_tcp_conn(host, port) == False:
      config.LOGGERS["RESOURCES"]["LOGGER_PREDATOR_DUMMY"].get_logger().critical("Wait " + str(config.SLEEP_THREAD_RESTART) + " to thread restart")
      ...
      config.LOGGERS["RESOURCES"]["LOGGER_PREDATOR_DUMMY"].get_logger().critical("Restarting thread")
      start_dummy(host, port)
    else:
      config.LOGGERS["RESOURCES"]["LOGGER_PREDATOR_DUMMY"].get_logger().info("Server reachable, restart not needed")
